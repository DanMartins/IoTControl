...
def main(): 
  ...
  controle=PI_BRUNO(fatorKp,fatorTi,fatorKd)
  ...
  try:
    while True:
      ...
      if (tempociclo <= (masterClock - tempoLoop)):
        ...
        #control
        dSetpoint = dControl*0.01
        controle.setPoint(dSetpoint)
        #control inputs Feedback, Ts
        Control_Output = controle.update(VP_Position, tempociclo)
        if PCF8591: 
        ...
        if (tempoStorage <= (masterClock - tempoGrava)):
          ...
          # input Param
          controle.setParam(fatorKp,fatorTi,0.0)
          ...
          if MCP4725 or PCF8591:
            csr.execute("INSERT INTO dados (ajuste, velocidade, erro, Kp, Ki, Kd, Kmotor, tempo) VALUES(?,?,?,?,?,?,?,?)",(dSetpoint, VT_Speed, AO1,fatorKp,fatorTi,fatorKd, VP_Position,xstamp))
          else:
            csr.execute("INSERT INTO dados (ajuste, velocidade, erro, Kp, Ki, Kd, Kmotor, tempo) VALUES(?,?,?,?,?,?,?,?)",(dControl, feedBack, pwm,fatorKp,fatorTi,fatorKd, KGain,xstamp))
